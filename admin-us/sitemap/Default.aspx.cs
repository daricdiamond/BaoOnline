    ...
    private void UpdateXml()
    {
            ...
            using (XmlTextWriter writer = new XmlTextWriter(Server.MapPath("~/sitemap.xml"), Encoding.UTF8))
            {
                ...
                foreach (DataRow row in dtDM.Rows)
                {
                    ...
                    string linkURL = BaseView.repalce_UrlFriendly(convertTitleToUrl + "-" + idDanhMuc) + ".hxml";
                ...
                {
                    //tableSiteMap.Rows.Add(new object[] { "http://thammydiamond.com/"+ BaseView.GetStringFieldValue(row, "code").ToLower() + ".hxml", DateTime.Now.Year+"-"+DateTime.Now.Month+"-"+DateTime.Now.Day, "weekly" });
                    writer.WriteStartElement("url");
                    writer.WriteElementString("loc", _bv.serverUrl() + "/" + BaseView.GetStringFieldValue(row, "code").ToLower() + ".hxml");
                    writer.WriteElementString("lastmod", String.Format("{0:yyyy-MM-dd}", DateTime.Now));
                    writer.WriteElementString("changefreq", "daily");
                    writer.WriteElementString("priority", "1.00");
                    writer.WriteEndElement();
                ...
                {
                    DateTime dateT = BaseView.GetDateTimeFieldValue(r, "ngaydang");
