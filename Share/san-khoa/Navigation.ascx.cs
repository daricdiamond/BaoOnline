...
public partial class Share_san_khoa_Navigation : System.Web.UI.UserControl
{
    ...
    private void getMenuSimple()
    {
        ...
        string url = "", name = "";
        string currentURL = HttpContext.Current.Request.RawUrl.Trim();
        string html = "";
        html += "<li class='dropdown'><a class='dropdown-toggle'  href='/' > <i class='fa fa-home' ></i> Trang chủ </a></li>";
        foreach (DataRow row in dt.Rows)
        {
            url = BaseView.GetStringFieldValue(row, "url").ToLower();
            ...
            if (dt2.Rows.Count > 0)
            {
                ...
                foreach (DataRow row2 in dt2.Rows)
                {
                    ...
                    url = BaseView.GetStringFieldValue(row2, "url").ToLower();
                    ...
                    if (dt3.Rows.Count > 0)
                    {
                        ...
                        {
                            url = BaseView.GetStringFieldValue(row3, "url").ToLower();
