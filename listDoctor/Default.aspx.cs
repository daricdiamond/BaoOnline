...
public partial class listDoctor_Default : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            getChuyenKhoa();
            getBacSi();
            getData(ToSQL.SQLToInt(lsBacsi.SelectedValue));
        }
    }
    ...
    private void getBacSi()
    {
        ...
        lsBacsi.DataSource = dt;
        lsBacsi.DataBind();
        lsBacsi.Items.Insert(0, new ListItem("-- (chọn bác sĩ ) ---", "0"));
    }
    private void getData(int doctorId)
    {
        string sqlCommand = "select * from LichDoctor where idDoctor = " + doctorId;
        DataTable dt = _db.sqlGetData(sqlCommand);
        grvTaskNew.DataSource = dt;
        grvTaskNew.DataBind();
    }
    protected void lsBacsi_SelectedIndexChanged(object sender, EventArgs e)
    {
        getData(ToSQL.SQLToInt(lsBacsi.SelectedValue));
    }
    private void getChuyenKhoa()
    {
        ...
        ddlChuyenKhoa.Items.Insert(0, new ListItem("-- (tất cả bác sĩ ) ---", "0"));
    }
    protected void ddlChuyenKhoa_SelectedIndexChanged(object sender, EventArgs e)
    {
        string sqlCommand = "select * from Doctor where idChuyenKhoa = " + ToSQL.SQLToInt(ddlChuyenKhoa.SelectedValue) + " or 0 = " + ToSQL.SQLToInt(ddlChuyenKhoa.SelectedValue);
        DataTable dt = _db.sqlGetData(sqlCommand);
        lsBacsi.DataSource = dt;
        lsBacsi.DataBind();
        lsBacsi.Items.Insert(0, new ListItem("-- (chọn bác sĩ ) ---", "0"));
